...
export class ProcessLogBuffer {
    ...
    private trim(): void {
        ...
        // Track total length incrementally instead of calling snapshot() per iteration
        let totalLen = this.entries.reduce((sum, e) => sum + e.length, 0)
            + Math.max(0, this.entries.length - 1); // newline separators

        while (this.entries.length > 1 && totalLen > this.maxChars) {
            const removed = this.entries[0];
            this.dropOldest();
            totalLen -= removed.length + 1; // entry + newline separator
        }
